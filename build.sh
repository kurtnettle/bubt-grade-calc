...
prepareBuildFolder() {
	...
	mkdir -p "$TEMP_DIR/"
	cp -r assets/icons $TEMP_DIR/icons
	pnpm exec rollup -c
}
...
build_crx() {
	...
	mv "$TEMP_DIR.crx" "$OUTPUT_DIR/bubt-grade-calc-v${version}.signed.crx"
...
build_zip() {
	...
	pnpm exec web-ext build --source-dir=$TEMP_DIR/ --artifacts-dir=dist/ --filename="bubt-grade-calc-v${version}.xpi" --overwrite-dest
